...
const Chat = ({ isOpen, onClose, id, wallet, onSendMessage, messages }) => {
  const [message, setMessage] = useState("");
  const messagesEndRef = useRef(null);

  useLayoutEffect(() => {
    // Assuming getMessages and listenForMessage are defined elsewhere
    setTimeout(async () => {
      const msgs = await getMessages(`guid_${id}`);
      setMessages(msgs);
      scrollToEnd();

      listenForMessage(`guid_${id}`).then((msg) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const msg = await onSendMessage(`guid_${id}`, message);
      setMessage("");
      scrollToEnd();
    } catch (error) {
  ...
  return (
    ...
    >
      <div className="bg-white shadow-lg shadow-slate-900 rounded-xl w-11/12 md:w-4/5 h-[30rem] p-6 relative">
        ...
        <form onSubmit={handleSendMessage} className="h-18 w-full mt-3">
          <input
            type="text"
            value={message}
